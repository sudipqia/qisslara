...
<div class="list-icons" id="action_menu_{{$model->id}}">
	<div class="dropdown">
		...
		<div class="dropdown-menu dropdown-menu-right">
			<a href="{{ route('admin.testimonial.edit', $model->id )}}">
				<span class="dropdown-item"><i class="icon-pencil7"></i> 
					{{_lang('Edit') }}
				</span>
			</a>
			<span class="dropdown-item" id="delete_item" data-id="{{ $model->id }}" data-url="{{ route('admin.testimonial.destroy', $model->id )}}"><i class="icon-trash"></i>
				 {{_lang('delete')}} 
				</button></span>
		</div>
	</div>
