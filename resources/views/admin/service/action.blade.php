...
<div class="list-icons" id="action_menu_{{$model->id}}">
	<div class="dropdown">
		...
		<div class="dropdown-menu dropdown-menu-right">
			<a href="{{ route('admin.service.edit', $model->hash )}}">
				<span class="dropdown-item"><i class="icon-pencil7"></i> 
					{{_lang('Edit') }}
				</span>
			</a>
			<span class="dropdown-item" id="delete_item" data-id="{{ $model->id }}" data-url="{{ route('admin.service.destroy', $model->hash )}}"><i class="icon-trash"></i>
				 {{_lang('delete')}} 
				</button></span>
		</div>
	</div>
