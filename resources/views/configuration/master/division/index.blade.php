...
$js = ['configuration/master/division'];
@endphp
{{-- Available Modal Size = xs, sm, lg, full --}}
@extends('layouts.app', ['title' => _lang('division_configuraion'), 'modal' => 'lg'])
@section('page.header')
	...
	<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
		<div class="d-flex">
			<div class="breadcrumb">
				...
				<span class="breadcrumb-item active">{{ _lang('division_configuraion') }}</span>
			</div>
...
<div class="card border-top-success rounded-top-0" id="table_card">
	<div class="card-header header-elements-inline bg-light border-grey-300" >
		<h5 class="card-title">{{ _lang('division_configuraion') }}
@can('location.create')
		<button type="button" class="btn btn-outline alpha-info text-info-800 border-info-600 rounded-round" id="content_managment" data-url="{{route($route.'create')}}"><i class="icon-stack-plus mr-1"></i> {{ _lang('add_new_division') }}</button>
		@endcan
		</h5>
			...
			<table class="table content_managment_table" data-url="{{route($route.'datatable')}}">
				<thead>
					<tr>
						...
						<th>{{ _lang('short_name') }}</th>
						<th>{{ _lang('phone_code') }}</th>
						<th>{{ _lang('action') }}</th>
					</tr>
				</thead>
				<tbody>
