    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="form-group">
                {{ Form::label('name', _lang('case_stage') , ['class' => 'col-form-label required']) }}
                 {{ Form::hidden('target', 'case_stage', ['id' => 'target']) }}
                {{ Form::hidden('status', 1) }}
                {{ Form::text('name', isset($data['name']) ? $data['name'] : Null, ['class' => 'form-control', 'placeholder' =>  _lang('case_stage'), 'required' => '']) }}
            </div>
        </div>
    </div>
    <div class="row">
